...
{
    [Tracked(false)]
    public class PowerUpSnowballTwo : Entity
    {
		...
		{
			base.Update();
			PowerUpSnowball snowball = level.Tracker.GetEntity<PowerUpSnowball>();
			Player player = level.Tracker.GetEntity<Player>();
			...
			if (Input.Grab.Pressed && !SnowballOnScreen && snowball.airTime > 0f)
			{
